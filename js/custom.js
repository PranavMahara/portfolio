...
$(document).ready(function () {
    ...
    $(document).ready(function() {
        ...
        const $slides = $slider.find('.col-md-4');
        const $prevBtn = $('.slider-nav.prev');
        const $nextBtn = $('.slider-nav.next');
        let currentIndex = 0;
        const slidesToShow = 3;

        // Hide all slides except first three initially
        ...
        function updateSlider() {
            $slides.each(function(index) {
                if (index >= currentIndex && index < currentIndex + slidesToShow) {
                    $(this).fadeIn(400).css({
                        'transform': `translateX(${(index - currentIndex) * 33.333}%)`,
                        'display': 'block'
                    });
                } else {
                    $(this).fadeOut(400);
        ...
        // Update slider when filter is clicked
        $('.portfolio_filter a').click(function() {
            // Reset currentIndex when filter changes
            currentIndex = 0;
            
            // Wait for isotope animation to complete
            setTimeout(function() {
                const $visibleSlides = $('.portfolio-slider .col-md-4:visible');
